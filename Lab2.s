...
NextChar
        	LDRB        R0, [R5]		; Read a character to convert to Morse Code - load first value in R5, aka. the first letter of the string. 
		...
		MOV		R9, #4		; delay 4 extra spaces (7 total) between words
		BL		DELAY
		BEQ         ResetLUT

ProcessChar	
		BL		CHAR2MORSE	; convert ASCII to Morse pattern in R1	
		CLZ		R8, R0 ; count leading 0s 
		LSL		R0, R8 ; shift left
		;0x0000 0000
charLoop			
		LSLS	R0, #1 ; Shift the next most signifcant bit to the left
		
		BLCS 	LED_ON ; If the carry flag is 1, then branch to turn on LED
		BLCC	LED_OFF ; If the carry flag is 0, then branch to turn off LED
		MOV		R9, #1 ; Set the delay as 1 * 500 ms long
		BL 		DELAY ; Branch to Delay
		
		CMP		R0, #0 ;If we're not done iterating throught the character morse, then loop again to continue blinking
		BNE		charLoop
		
		BL		LED_OFF ; Turn off LED incase we turned it on last time
		MOV 	R9, #3 ; Set the delay to 3 * 500 ms long
		BL		DELAY 
		
		B NextChar ;  Branch to get the next character
		
		
		
		; while R0 Is not equal to 0
			; If MSB is 1, Turn on LED using TurnOn LED subroutine
			; IF MSB is 0, Turn Off LED using TurnOFF LED subroutine,
			; Then shift R0 to the left by 1;
			; Set R9 to 1, Call delay branch
		;If R0 is equal to 0
		; Set R9 to 0, Branch to Delay		
...
CHAR2MORSE	STMFD		R13!,{R14}	; push Link Register (return address) on stack
		...
		LDR 		R6, =MorseLUT
		SUBS 		R0, #0x41 ; Subtract 0x41 from the ASCII letter
		; MOV  		R0, LSL #1 ; multiply offset by 2
		MOV 		R7, #2 
		MUL 		R0, R7
		LDRH 		R0, [R6, R0] ; read from lut 
...
; LED_ON: Turns on the LED connected to pin P.1.28
LED_ON 	   	
		push 		{r3-r4}		; preserve R3 and R4 on the R13 stack
		
		; ... insert your code here
		
		MOV			R3, #0xA0000000 ; turn on LED at pin P.1.28
		STR			R3, [R4, #0x20] ; store toggled value, 0xA0000000 into address 0x2009C000 offset 0x20 = 0x2009C020  		
		pop 		{r3-r4}
...
; LED_OFF: Turns off the LED connected to pin P.1.28
LED_OFF	
		STMFD		R13!,{R3, R14}	; push R3 and R14, the Link Register (return address) on stack
		; ... insert your code here
		
		MOV			R3, #0xB0000000 ; turn off LED at pin P.1.28
		STR			R3, [R4, #0x20] ; store toggled value, 0xB0000000 into address 0x2009C000 offset 0x20 = 0x2009C020  		
		LDMFD		R13!,{R3, R15}	; pushes R3 into R3 and pushes R14 value into R15 - restore R3 and LR to R15 the Program Counter to return

;	Delay 500ms * R0 times
;	Use the delay loop from Lab-1 but loop R0 times around
;
DELAY			STMFD		R13!,{R2, R14}
MultipleDelay		TEQ		R9, #0		; test R0 to see if it's 0 - set Zero flag so you can use BEQ, BNE
			
			; ... insert your code here
			MOV		R10, #0x2C2B  
			MOVT	R10, #0x000A ; store counter for 500ms delay
...
;
InputLUT	DCB		"NSFLP", 0	; strings must be stored, and read, as BYTES
